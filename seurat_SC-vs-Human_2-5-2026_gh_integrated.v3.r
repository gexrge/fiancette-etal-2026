...
# check ribosomal gene expression
integrated$percent.rb <- PercentageFeatureSet(integrated, pattern = "^RP[LS]")
FeaturePlot(integrated, reduction = "umap.harmony", features = "percent.rb", raster = FALSE)
VlnPlot(integrated, group.by = "beta_cell", features = c("percent.rb", "percent.mt", "nFeature_RNA", "nCount_RNA"), ncol = 2) + NoLegend()
FeatureScatter(integrated, group.by = "source", feature1 = "nCount_RNA", feature = "nFeature_RNA", raster = FALSE)
...
integrated@meta.data <- integrated@meta.data %>%
  mutate(pcc_clusters.reps = case_when(
      ...
      cell_type == "alpha" ~ paste("alpha", source, sep = "_"),
...
integrated$pcc_clusters.reps <- factor(
  ...
  levels = c(
    ...
    "alpha_bandesh", "alpha_fasolino", "alpha_kang",
    "alpha_H1", "alpha_H3", 
    "primary-delta", "sc-delta",
    "ductal", "acinar"
  )
...
make_pcc_step <- function(step, pcc, pcc_clust, beta) {
  ...
  if (step == "top3000-beta-vfs") {
    ...
    hvgs <- lapply(levels(pcc$cell_type), function(ct) {
      ...
      VariableFeatures(obj, method = "sctransform", nfeatures = 3000)
    })
    ...
    vfs <- VariableFeatures(pcc.ds, method = "sctransform", nfeatures = 3000)
...
integrated$expr.sets <- case_when(
  integrated$cell_type == "beta" ~ integrated$beta_cell,
  integrated$cell_type == "alpha" ~ integrated$alpha_cell,
  integrated$cell_type == "delta" ~ integrated$delta_cell,
  TRUE ~ NA
)

avg.expr.sets <- AverageExpression(integrated, group.by = "expr.sets", assays = "RNA", layer = "data", verbose = FALSE)$RNA
...
sbc.vs.pbc <- sbc.vs.pbc[order(sbc.vs.pbc$p_val_adj, decreasing = FALSE),]
...
sbc.vs.pbc_rna <- sbc.vs.pbc_rna[order(sbc.vs.pbc_rna$p_val_adj, decreasing = FALSE),]

# rename columns for clarity and create gene column
sbc.vs.pbc_rna <- dplyr::rename(sbc.vs.pbc_rna, pct.sbc = pct.1, pct.pbc = pct.2)
sbc.vs.pbc_rna$gene <- row.names(sbc.vs.pbc_rna)

write.table(
  sbc.vs.pbc_rna,
  file.path(outdir, "FindMarkers_SBC-vs-PBC_output.RNA.tsv"),
  quote = FALSE,
  row.names = FALSE,
  sep = "\t"
)
...
sbc.vs.pbc_rna %>% 
  ...
  arrange(desc(avg_log2FC)) %>% 
  write.table(
    file.path(outdir, "FindMarkers_SBC-vs-PBC_output.RNA.rnk"),
    quote = FALSE,
    row.names = FALSE,
    col.names = FALSE,
    sep = "\t"
...
integrated.markers.beta_cell <- FindAllMarkers(integrated, assay = "RNA", group.by = "beta_cell", verbose = FALSE)
