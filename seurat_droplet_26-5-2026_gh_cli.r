...
for (exp in names(crdir_filt)) {
  ...
  # ---- MALAT1 filtering ----
  # apply malat1 thresholding per experiment: https://github.com/BaderLab/MALAT1_threshold
  sobj.filt <- CreateSeuratObject(counts = sobj.filt, project = nobj)
  
  sobj.filt <- seurat_pipe_20.2.2026_gh(
    sobj = sobj.filt, 
    FindNeighbors.dims = FindNeighbors.dims,
    FindClusters.res = FindClusters.res
  )
  
  norm_counts <- GetAssayData(sobj.filt, assay = "SCT", layer = "data")["MALAT1",]
  threshold <- define_malat1_threshold_ggplot2(norm_counts)
  malat1_threshold <- norm_counts > threshold
  sobj.filt$malat1_threshold <- malat1_threshold
  sobj.filt$malat1_threshold <- factor(sobj.filt$malat1_threshold, levels = c(TRUE, FALSE))
  print(DimPlot(sobj.filt, reduction = "umap", group.by = "malat1_threshold"))
  good_cells <- colnames(sobj.filt)[malat1_threshold]
  sobj.filt <- subset(sobj.filt, cells = good_cells)
